...
{
	$charcheck = mysql_query("SELECT * FROM `Slayer` WHERE `PlayerID` = '$user_id' AND `Active` = 'ACTIVE'");
?>
...
echo "</td></tr></table></center>";
	...
	if(!$id) {
		echo "
				<script language='javascript'>
				alert('Error: The item you bought is not there.')
	...
	if($query['Type'] == 2)
	{
		echo "
				<script language='javascript'>
				alert('Error: You can not buy this item.')
				history.back(1)
			</script>
		";
	}
...
<div style="background:url(../images/servidor.png) no-repeat;"><div class='memin'></div><div class='memin'></div><br></br><br></br></span>
    ...
    <div>
      <td class="listtitle" colspan="2"><center>
        <br />
        <center>
          <img src = "/images/market/<?=$query['PID'] ?>.jpg" alt="<?=$query['Name'] ?>" /><br />
        </center>
        <span class="mamon"><center><br />
        Item:
        <?=$query['Name'] ?>
        </b>
        <p class="mamon">Price:
        ...
        <p><span class="mamon">Descripcion:
        <?=$query['Text'] ?>
        <br />
        </span></p></center><br />
      </p></td>
      ...
      <form method="post" action="market_buy_do.php" onsubmit="return chk_form();" >
        <input type="hidden" name="ID" value="<?=$query['ID'] ?>" />
        <input type="hidden" name="Security" value="1" />
        <tr>
          <td class="list" align="right"><font color=black>Character:</td>
          <td><select name="charname">
<?
	while($row = mysql_fetch_assoc($charcheck)){
	$char = $row['Name'];
	$Race = $row['Race'];
		if($query['Race'] == "COMMON")
		{
			echo "<option value='$char|$Race'>[$Race] $char</option>";
		}
...
?></select></td>
        ...
        <tr>
          <td class="listtitle" align="right" colspan="2"><input id="btn_01" type='submit' value="Buy" class="form-button" /></td>
        </tr>
      </form>
<tr><br></p>
</div>
